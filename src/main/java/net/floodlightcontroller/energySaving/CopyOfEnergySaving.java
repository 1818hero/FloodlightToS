...
public class CopyOfEnergySaving implements IFloodlightModule {
	
	protected static Logger log = LoggerFactory.getLogger(CopyOfEnergySaving.class);
	...
	private static boolean flag=true;  //这个flag为true表示不进行判断节能的策略
	...
	*/
	...
	public void energySaving(){
		...
		while(iter.hasNext()){
			...
			while(iterator.hasNext()){
	        	...
	        	//(默认端口号是1)则不做任何的处理
	        	if(portReply.getPortNumber()==-2||portReply.getPortNumber()==1){  
	        		continue;
	        	}
	        	...
	        	if(!flag){        //flag初始化为true，节能策略不执行,用来初始化lastTimePortTraffic
	        		
	        		long portByteRate=currentPortTraffic-lastTimePortTraffic.get(dpid).get(portNumber);
	        		
	        		//portByteRate_MB表示在间隔30s内发送的数据总数，以字节的单位进行计算
	        	    double portByteRate_MB=portByteRate/(1024.0*1024.0);
	        		
	        	    log.info("the port speed in MB is {}",portByteRate_MB);
	        	    
	        		if( portByteRate_MB < 107.1 ){
	 ...
	 */
	public boolean canBeTurnDown(long srcId,short portNumber){
		...
		while(iter.hasNext()){             //寻找出要关闭的前向链路，break语句前的forwardLink是要寻找的链路
			...
			if(forwardLink.getSrcPort()==portNumber){
				break;
			}
			forwardLink=null;
		}
		...
		backwardLink=new Link(dstId,dstPortNumber,srcId,portNumber);  //实例化后向向链路
		...
		return isReachable(srcId,dstId);
